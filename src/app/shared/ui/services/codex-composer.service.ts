...
export class CodexComposerService {
  ...
  cloneLayer(svg: SVGSVGElement): SVGSVGElement {
    return svg.cloneNode(true) as SVGSVGElement;
  }
  compose(layers: SVGSVGElement[]): SVGSVGElement {
    if (!layers.length) {
      throw new Error('[CodexComposer] No SVG layers provided.');
    }
  ...
  private mergeStyles(defs: SVGDefsElement, layers: SVGSVGElement[]): void {
    const css: string[] = [];
    for (const layer of layers) {
      layer.querySelectorAll('style').forEach((style) => {
  ...
  private mergeGroups(root: SVGSVGElement, layers: SVGSVGElement[]): void {
    ...
    root.querySelectorAll('g[id]').forEach((group) => {
      rootGroups.set(group.id, group as SVGGElement);
    });
    for (const layer of layers) {
      Array.from(layer.children)
        ...
        .forEach((group) => {
          const id = group.id;
          if (id && rootGroups.has(id)) {
            const target = rootGroups.get(id)!;
